...
#define COMPARE_BUFF_LEN         (2048)





static RingBuffFd     *ring_buff_compare_ = NULL;
static OldPhyCodeRef   old_code_ref_;
static WTFreqCodeType code_ref_temp_;
static RecvAudioType pcm_buf_compare_[FREQ_ANA_BUF_SIZE];
static int compare_pcm_buf_w_addr_ = 0;
...
static int GetNextFreqCode(WTFreqCodeType *data)
{
  ...
  int ret;
  BUF_LOCK();
  while ((ret = RingBuffReadData(ring_buff_compare_, &code, sizeof(WTFreqCodeType))) != 0) {
    if (code == old_code_ref_.code_) {
      ...
      if (code == code_ref_temp_) {
        ...
        code_ref_temp_ = REF_MARK_INIT_DATA;
        int old_num = WTPhyAnalysisNumToRealNum(old_code_ref_.code_num_);
        if (old_num > old_code_ref_.already_num_) {
          //WtLog(" #%ld ", old_freq_code_ref_.freq_code_);
          *data = code;
...
{
  ring_buff_compare_ = RingBuffCreate(COMPARE_BUFF_LEN);
  if (ring_buff_compare_ == NULL) {
    return -1;
  }
  BUF_LOCK_INIT();
  old_code_ref_.already_num_ = 0;
...
{
  BUF_LOCK();
  RingBuffDestroy(ring_buff_compare_);
  old_code_ref_.already_num_ = 0;
  old_code_ref_.code_num_ = 0;
  old_code_ref_.code_ = REF_CODE_INIT_DATA;
  code_ref_temp_ = REF_CODE_INIT_DATA;
...
void WTRecvPhyLayerSendPcmForCompare(const RecvAudioType * pcm, int pcm_len)
{
  ...
  if (compare_pcm_buf_w_addr_ != 0) {
    if (pcm_len < FREQ_ANA_BUF_SIZE - compare_pcm_buf_w_addr_) {
      memcpy(&pcm_buf_compare_[compare_pcm_buf_w_addr_], pcm, sizeof(RecvAudioType)*pcm_len);
      compare_pcm_buf_w_addr_ += pcm_len;
      return;
    }
    memcpy(&pcm_buf_compare_[compare_pcm_buf_w_addr_], pcm, sizeof(RecvAudioType)*(FREQ_ANA_BUF_SIZE - compare_pcm_buf_w_addr_));
    pcm_r_addr += FREQ_ANA_BUF_SIZE - compare_pcm_buf_w_addr_;
    compare_pcm_buf_w_addr_ = 0;
    if (WTPhysicalPcmDecode(pcm_buf_compare_, FREQ_ANA_BUF_SIZE, &code) == 0) {
      BUF_LOCK();
      //WtLog(" %ld ", code);
      RingBuffWriteData(ring_buff_compare_, &code, sizeof(WTFreqCodeType));
      BUF_UNLOCK();
    }
  }
  ...
  if (pcm_r_addr<pcm_len) {
    memcpy(pcm_buf_compare_, pcm + pcm_r_addr, sizeof(RecvAudioType)*(pcm_len - pcm_r_addr));
    compare_pcm_buf_w_addr_ = pcm_len - pcm_r_addr;
  }
}
